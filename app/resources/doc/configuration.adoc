...
Our configuration is defined as a map, in link:/sources/resources/config.edn[resources/config.edn].

This is a standard Clojure *EDN* file that contains a few custom **tag literal**s
that combine to make the EDN format more suitable for configuration. These tag literals are defined in one of our most popular libraries, https://github.com/juxt/aero[Aero]footnote:[You can find more details about Aero on https://github.com/juxt/aero[Aero's GitHub page]].
...
One of the entries in the configuration map is `:edge/httpd`. Let's see what the value is when we read it with the `profile` argument above set to `:dev`:

[source,clojure]
----
include::config:dev:edge/httpd[]
----
...
[source,clojure]
----
include::config:prod:edge/httpd[]
----
...
====
For example, we can copy in the port of our httpd server like this:

----
#ref [:edge/httpd :edge.httpd/port]
----
====
...
====
For example, if our `:edge/httpd` component is to depend on `:edge/event-bus`, we declare the relationship like this:
----
:edge/httpd {:edge/event-bus #ig/ref :edge/event-bus}
----
====
