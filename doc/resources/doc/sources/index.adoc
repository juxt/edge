...
|`bin`|Shell scripts
|`phonebook`|An example application of phonebook written in Clojure (API) and JavaScript (front-end)
|`phonebook-app`|A richer replacement frontend for `phonebook`, written in ClojureScript.
|`phonebook-graphql`|A GraphQL API for the `phonebook` example.
|`graphql-ws`|An example of internal library, incubating in Edge.
...
Edge contains a phonebook which serves to demonstrate how to build an
API with [.yada]#yada# with content negotiation. The `text/html`
representation of the API's entity bodies is also somewhat useable.
